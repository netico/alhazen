...
const generateChartData = (data, settings) => {
  ...
  if (!settings.serieField || !fields.includes(settings.serieField)) {
    return { error: 'Serie field is empty or has a wrong value', data: null };
  }
  ...
  if (settings.trendBy && fields.includes(settings.trendBy)) {
    ...
    if (fields.length > 1) {
      return { error: 'Trend field is empty or has a wrong value', data: null };
    }
  }
...
exports.barchart = (rawData, sheet) => {
  ...
  if (settings === undefined) {
    logger.log('error', 'Settings not defined for barchart %s', sheet.viewApi);
    return { error: 'Settings not defined for the chart', result: null };
  }

  const { error, data } = generateChartData(rawData, settings);
  if (error) {
    logger.log('error', 'Error in generateChartData for barchart %s; Error: %s', sheet.viewApi, error);
  ...
  const chart = {
    ...
    options: {
      title: {
        display: true,
        text: sheet.viewName.toUpperCase(),
        fontSize: 28,
      },
      legend: {
...
exports.piechart = (rawData, sheet) => {
  ...
  if (!settings.serieField || !fields.includes(settings.serieField)) {
    logger.log('error', 'Error generating piechart %s; Error: Serie field is empty or has a wrong value', sheet.viewApi);
    return { error: 'Serie field is empty or has a wrong value', data: null };
  }
  ...
  if (fields.length > 1) {
    logger.log('error', 'Too fields for generating piechart %s; Error: piechart required only two fields', sheet.viewApi);
    return { error: 'Piechart required only two fields', result: null };
  }
  const data = {
    labels: [],
    datasets: [{
...
exports.linechart = (rawData, sheet) => {
  const settings = mergeSettings(sheet.settings, sheet.typeApi);
  if (settings === undefined) {
    logger.log('error', 'Settings not defined for linechart %s', sheet.viewApi);
    return { error: 'Settings not defined for the chart', result: null };
  }
